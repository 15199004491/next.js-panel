...
function MenuItem({ item, openMenus, setOpenMenus, currentPath }: { item: MenuItem; openMenus: Set<string>; setOpenMenus: React.Dispatch<React.SetStateAction<Set<string>>>; currentPath: string }) {
  ...
  if (item.children && item.children.length > 0) {
    return (
      <li key={item.name}>
        <button
          onClick={toggleMenu}
          className={`flex items-center justify-between w-full px-4 py-3 rounded-lg transition-colors ${isActive ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-blue-50 hover:text-blue-600'}`}
        >
              ...
              return (
                <li key={child.name}>
                  <Link
                    ...
                    className={`flex items-center gap-3 px-4 py-2 ml-8 rounded-lg transition-colors ${isChildActive ? 'bg-blue-50 text-blue-600' : 'text-gray-500 hover:bg-blue-50 hover:text-blue-600'}`}
                  >
  ...
  return (
    <li key={item.name}>
      <Link
        href={item.href || '#'}
        className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${isActive ? 'bg-blue-50 text-blue-600' : 'text-gray-600 hover:bg-blue-50 hover:text-blue-600'}`}
      >
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={iconPaths[item.icon]} />
        </svg>
