...
export const mockCreateResaleProperty = (data: Omit<ResaleProperty, 'id' | 'createdAt' | 'updatedAt'>): ResaleProperty => {
  const newItem: ResaleProperty = { ...data, id: Date.now().toString(), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
...
export const mockUpdateResaleProperty = (id: string, data: Partial<ResaleProperty>): ResaleProperty | undefined => {
  ...
  if (index !== -1) {
    mockResaleProperties[index] = { ...mockResaleProperties[index], ...data, updatedAt: new Date().toISOString() };
